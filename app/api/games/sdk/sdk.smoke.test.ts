...
import { users, travelerProfiles, agentKeys, rooms, gameDefs } from "@db/schema";
...
afterAll(async () => {
  ...
  if (createdUserIds.length > 0) {
    ...
    await db
      .delete(agentKeys)
      .where(inArray(agentKeys.userId, createdUserIds));
  ...
  it("rejects out-of-range UGC params", async () => {
    ...
    await expect(
      caller.game.createDef({
        ...
        seats: 4,
        params: { rounds: 5, min: 0, max: 100, targetRatio: 1.6, scoreWin: 2, scoreSecond: 1 },
      }),
    ).rejects.toThrow();
  });
});
...
describe("sdk pollDuel UGC full pipeline (smoke)", () => {
  it(
    "createDef → create room → human + key agent + bots → 7 rounds → custom rewards",
    async () => {
      ...
      expect(defs.some((d) => d.id === "poll-duel-core" && d.isOfficial)).toBe(true);
      ...
      // ③ Key Agent 入座
      const { key, agentId } = await callerA.agent.register({ name: "sdk-smoke-bot" });
      const joinedAgent = await callerAnon.agent.gatewayJoin({ key, code: roomCode });
      ...
      const started = await callerAnon.agent.gatewayObserve({ key, code: roomCode });
      ...
      while (Date.now() < deadline) {
        const view = await callerAnon.agent.gatewayObserve({ key, code: roomCode });
        lastView = view;
