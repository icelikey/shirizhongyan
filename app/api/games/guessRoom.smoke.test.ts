...
import { users, travelerProfiles, agentKeys, rooms } from "@db/schema";
...
describe("guess room full pipeline (smoke)", () => {
  it(
    "create → 2 humans + 1 agent join → bots fill → 5 rounds → settle",
    async () => {
      ...
      for (let i = 0; i < 20; i++) {
        const aOk =
          ...
          (profileA.recordsJson as { guess?: { played?: number } } | null)
            ?.guess?.played === 1;
        const bOk =
          profileB &&
          (profileB.recordsJson as { guess?: { played?: number } } | null)
            ?.guess?.played === 1;
        if (aOk && bOk) break;
        await sleep(500);
      ...
      expect(
        keysAfter.find((k) => k.id === agentId)?.lastUsedAt,
      ).not.toBeNull();
    },
    300_000,
  );
